...
class GlCdbListController extends Controller
{
    // Read data
    public function index(Request $request)
    {
        ...
        $records = GlCdbList::whereIn('transaction_status', $transaction_statuses)
        ...
        if (in_array('UT', $transaction_statuses) || in_array('U', $transaction_statuses)) {
            if (in_array('transmit', $approvalAccessArray)) {
                $records = $records->filter(function ($record) use ($user_id) {
                    ...
                    $checkAmount = round($record->check_amount, 2);
        
                    $amountRange = AmountRange::where('range_from', '<=', $checkAmount)
                        ->where('range_to', '>=', $checkAmount)
                        ->first();
    ...
    public function forward(Request $request)
    {
        ...
        $record = GlCdbList::where('doc_no', $doc_no)
